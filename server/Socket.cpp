...
void Socket::setSocket(int port) {
  ...
  ret = bind(m_listen_fd, reinterpret_cast<sockaddr *>(&m_addr),
             sizeof(sockaddr));
  if (ret != 0) {
  ...
  if (ret != 0)
    std::cout << "Listening on FD " << m_listen_fd << " failed miserably"
              << std::endl;
  else
    std::cout << "LISTENING SUCCESS on port : " << PORT << std::endl;
}

int Socket::getSocketFd(void) { return (m_listen_fd); }
