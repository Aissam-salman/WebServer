...
std::string decode_chunked(std::string& body_raw) {
    std::string body;
    while (true) {
    std::getline(body_raw):
        std::string size_line = body_raw.read_line();   // reads up to and consumes \r\n
        size_t size = parse_hex(size_line);          // strtol(s.c_str(), NULL, 16)
        if (size == 0) {
            body_raw.read_line();                        // consume the final empty line
            break;
        }
        std::string chunk = body_raw.read_n_bytes(size);
        body_raw.read_line();                            // consume the \r\n after the chunk
        body += chunk;
    }
    return body;
}
