...
/*   Updated: 2026/07/12 22:56:45 by salman           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Client::Client(int fd) : _request("8080", "0", "0000", "www") {
    _max_body_size = 20000000;
    _status = READING;
    _offset_send = 0;
    _poll_listen.fd = fd;
...
Client::Client(int fd, Request rq) : _request(rq) {
    _max_body_size = 20000000;
    _status = READING;
    _offset_send = 0;
    _poll_listen.fd = fd;
...
Client::Client(int fd, Request rq, long max_body_size) : _request(rq) {
...
bool Client::isRequestCompleted(void) {
    ...
    if (header_contentLength_pos != std::string::npos &&
        ...
        std::string lenString =
            _buffer_read.substr(len_start, len_end - len_start);
        ...
        if (len > _max_body_size) {
            _status = TRASH;
            _counter_trash =
                static_cast<long>(len) - static_cast<long>(body_size);
        }
